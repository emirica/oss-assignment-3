...
int main(int nargc, char *argv[])
{
    ...
    // First do some standard tests to verify the correctness of the algorithm
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        botan_out << file_sizes[i - 1] << " - Botan Twofish ===\n";
        ...
        std::cout << "[ " << diff << " us] Correctness: Test for file " << file_sizes[i - 1] << " ended\n";
    ...
    // DataSink_Stream allows us to test with big files
    for (int i = 1; i <= 6; i++) {
        std::stringstream ss;
        // Open the input file
        ss << i;
        ...
        input_enc.open((input_name + ss.str() + "_nohash.enc").c_str(), ios::out);
        if (!input_enc) {
            std::cerr << "ERROR: couldn't open " << TEST_FILENAME << i << "_nohash.enc" << std::endl;
            return 1;
        }
        input_dec.open((input_name + ss.str() + "_nohash.dec").c_str());
        if (!input_dec) {
            std::cerr << "ERROR: couldn't open " << TEST_FILENAME << i << "_nohash.dec" << std::endl;
            return 1;
        }
        uint64_t start = get_str_time(timebuf);
        botan_out << "\t[" << timebuf <<"] === Start: Speed ";
        ...
        // Required header for the decryption phase
        input_enc << ENC_HEADER << std::endl;
        Pipe encryptor(get_cipher(algo + mode, key, iv, ENCRYPTION),
        ...
        input_enc.close();
        input_enc.open((input_name + ss.str() + "_nohash.enc").c_str(), ios::in);
        if (!input_enc) {
            std::cerr << "ERROR: couldn't open " << TEST_FILENAME << i << ".enc for read" << std::endl;
            return 1;
        }
        ...
        Pipe decryptor(get_cipher(algo + mode, key, iv, DECRYPTION),
                       new DataSink_Stream(input_dec)
                      );
        decryptor.start_msg();
        input_enc >> decryptor;
        decryptor.end_msg();
        ...
        input_enc.close();
        input_dec.close();
        std::cout << "[ " << diff << " us] Speed: Test for file " << file_sizes[i - 1] << " ended\n";
